...
class CorrelationTracker(Tracker):
    ...
    def __init__(self, detection_rate=3, **kwargs):
    ...
    def build_tracklet(self, frame):
        ...
        for label, xmin, ymin, xmax, ymax in self.detection_history[-1]:
            ...
            prev_bbox = prev_frame[ymin: ymax + 1, xmin: xmax]
            # Step 1: Extract new_bbox from current frame with the same coordinates
            new_bbox = frame[ymin: ymax + 1, xmin: xmax]
            ...
            xm = (xmax - xmin) / 2
            ym = (ymax - ymin + 1) / 2
            mg = matching * gaussian([ymax - ymin + 1, xmax - xmin], xm, ym, xmax - xmin, ymax - ymin + 1)
            x, y = np.unravel_index(np.argmax(mg), mg.shape)[::-1]
            ...
            detections.append((label, min(max(xmin + xm, 0), frame.shape[1] - 1), 
                               min(max(ymin + ym, 0), frame.shape[0] - 1), 
                               min(max(xmax - xm, 0), frame.shape[1] - 1), 
                               min(max(ymax - ym, 0), frame.shape[0] - 1)))
